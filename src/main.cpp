...
unsigned int nStakeMinAge = 23 * 60 * 60;	// minimum age for coin age: 23 Hours
unsigned int nStakeMaxAge = 27.4 * 24 * 60 * 60;	// stake age of full weight: 27.4 Days (Lunar Cycle)
unsigned int nStakeTargetSpacing = 90;			// 60 sec block spacing

int64 nChainStartTime = 1484973722;
int nCoinbaseMaturity = 27;
...
const string strMessageMagic = "HalfMoon Signed Message:\n";
...
bool CTransaction::IsStandard() const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        // 2017-04-19 Adriano https://bitcointalk.org/index.php?action=profile;u=112568
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    {
        if(nHeight > 4)
        nSubsidy = 1537600000 * COIN;  // 8% for exchange listing, coinswap, and airdrops (15.3 Billion)
        return nSubsidy + nFees;   
    }

    // Subsidy is cut in half set by the duration of a lunar sunset to sunrise in earth days ~29.5 days
    nSubsidy >>= (nHeight / 42480);
    // 1st - 21,190,000,000
    // 2nd - 10,620,000,000 [total = 31,810,000,000]
    // 3rd -  5,310,000,000 [total = 37,120,000,000]
    // 4th -  2,655,000,000 [total = 39,775,000,000]
    // 5th -  1,327,500,000 [total = 41,102,500,000]
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    int64 nMineoutCoinYear;
    nMineoutCoinYear = MINEOUT_MINT_PROOF_OF_STAKE;

	int64 nSubsidy = nCoinAge * nRewardCoinYear / 365;
	if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfStakeReward(): create=%s nCoinAge=%"PRI64d" nBits=%d\n", FormatMoney(nSubsidy).c_str(), nCoinAge, nBits);

    if(nBestHeight > POS_MINEOUT_BLOCK)
    {
        nSubsidy = nCoinAge * nMineoutCoinYear / 365;
    if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfStakeReward(): create=%s nCoinAge=%"PRI64d" nBits=%d\n", FormatMoney(nSubsidy).c_str(), nCoinAge, nBits);
    }
...
static const int64 nTargetTimespan = 90;  
static const int64 nTargetSpacingWorkMax = 90; 
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake) 
{
    ...
    // Genesis block
        ...
        int64_t _nTargetTimespan = CountBlocks * nTargetSpacingWorkMax;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in HalfMoon
    bool fStrictPayToScriptHash = true; // Always active in HalfMoon
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "HalfMoon", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nStakeMaxAge = 8 * 60 * 60; // test net min age is 8 Hours
		nModifierInterval = 90; // test modifier interval is 1.5 Minutes
        nCoinbaseMaturity = 10; // test maturity is 10 blocks
        nStakeTargetSpacing = 1 * 90; // test block spacing is 1.5 Minutes
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "HalfMoon comes into view as of January 2017 1484973722";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1484973722;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1036819;

        if(fTestNet == true)
        {
        block.nTime    = 1484973922;
        block.nNonce   = 1260045;
        }
/*
...
*/
        assert(block.hashMerkleRoot == uint256("0xc535577d27b5c4b98da44b6ad0b857808e528aa86772521d55737550bb2dcf64"));
...
unsigned char pchMessageStart[4] = { 0xca, 0x18, 0xe1, 0x09 };
